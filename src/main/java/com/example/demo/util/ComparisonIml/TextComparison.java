package com.example.demo.util.ComparisonIml;
...
public class TextComparison implements TextComparisonStrategy {
    private String tokens; //字符串
    private final BigInteger strSimHash;//字符产的hash值
    ...
    public TextComparison(String tokens) {
        this.tokens = tokens;
        this.strSimHash = this.simHash();
    }
    ...
    private TextComparison(String tokens, int hashbits) {
        this.tokens = tokens;
     ...
     * @return
     */
    private int hammingDistance(TextComparison other) {
    ...
    public int hammingDistance(BigInteger b1,BigInteger b2) {
        BigInteger m = new BigInteger("1").shiftLeft(this.hashbits).subtract(
                new BigInteger("1"));
        BigInteger x = b1.xor(b2).and(m);
        int tot = 0;
        while (x.signum() != 0) {
            tot += 1;
            x = x.and(x.subtract(new BigInteger("1")));
...
//        String s1="这是一个s1字符串";
//        String s2="这是一个s2的字符串";
//        String s3="删除已提交的报告，该报告仍然会显示在报告列表中";
//        String s4="该报告仍然会显示在报告列表中，即使删除已提交的报告";
//        TextComparison hash1 = new TextComparison(s1, 64);
//        TextComparison hash2 = new TextComparison(s2, 64);
//        TextComparison hash3 = new TextComparison(s3, 64);
//        TextComparison hash4 = new TextComparison(s4, 64);
//        BigInteger b1=new BigInteger("8598224380639515977");
//        BigInteger b2=new BigInteger("8598224380773733705");
...
//        System.out.println(  hash1.getSemblance(hash3) );
        ...
        for (int i = 0; i < 256; i++) {
            bytes[i]= (byte) (t.charAt(i)-'0');
        }
//        System.out.println(  hash4.getSemblance(hash3) );
//        System.out.println(  hash3.getSemblance(b1,b2));
